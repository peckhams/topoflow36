...
#        % conda install geopandas  (includes numpy)
...
class catchment:
    ...
    def read_data(self):
        ...
        #---------------------------------  
        # Read data into geo data frames
        #---------------------------------
        self.catchment_df = gpd.read_file( self.catchment_data_file )
        self.nexus_df     = gpd.read_file( self.nexus_data_file )
        nc = len(self.catchment_df)
    ...
    def get_bounding_box(self, cat_id_str, buffer_sec=0, REPORT=False):
        ...
        #         Coordinates are Geographic, not Albers Conic.
        #--------------------------------------------------------------
        # self.catchment_df.columns
        # Index(['id', 'area_sqkm', 'toid', 'geometry'],
        #        dtype='object') 
        #--------------------------------------------------------------
        # Other options:
        ...
        #----------------------------------
        # env = g.envelope
        # x,y = env.exterior.coords.xy
        #---------------------------------------------------------------   
        df = self.catchment_df
        record = df[df['id'] == cat_id_str]
        if (len(record) == 0):
        ...
        bounds = record['geometry'].bounds.values[0]
        self.minlon = bounds[0]
        self.minlat = bounds[1]
        self.maxlon = bounds[2]
        self.maxlat = bounds[3]     
...
#             print()
        ...
        out_file = cat_id_str + '_DEM.tif'       
        out_unit = gdal.Warp( out_file, vrt_unit,
...
#         print()
        ...
        if (SAVE_DEM):
            ...
            outRaster = driver.Create(out_file, ncols, nrows, nbands, data_type)
    ...
    def get_outlet_coords(self, cat_id_str, REPORT=False):
        ...
        #----------------------------------------------------------------
        # self.catchment_df.columns
        # Index(['id', 'nexus_type', 'toid', 'geometry'], dtype='object')
        #----------------------------------------------------------------
        df1 = self.catchment_df     
        df2 = self.nexus_df
  
        rec1 = df1[df1['id'] == cat_id_str]
        if (len(rec1) == 0):
            print('SORRY, cat_id not found:', cat_id_str)
            print('Returning.')
            print()
            return -1
        nexus_id_str = rec1['toid'].values[0]
        self.nexus_id_str = nexus_id_str
        
        rec2 = df2[df2['id'] == nexus_id_str]
        if (len(rec2) == 0):
            print('SORRY, nexus_id not found:', nexus_id_str)
            print('Returning.')
            print()
            return -1
        nexus_type = rec2['nexus_type'].values[0]
        if (nexus_type != 'junction'):
            print( 'WARNING: nexus_type =', nexus_type )

        lon = rec2['geometry'].values.x[0]
        lat = rec2['geometry'].values.y[0]
        self.outlet_x = lon
        self.outlet_y = lat
        
        coords = np.array([lon, lat], dtype='float64')
    ...
    def get_area_sqkm(self, cat_id_str, REPORT=False):    
        ...
        df = self.catchment_df
        rec = df[df['id'] == cat_id_str]
        ## if (rec.empty):
        if (len(rec) == 0):
            print('SORRY, cat_id not found:', cat_id_str)
            return -1

        self.area_sqkm = rec['area_sqkm'].values[0]
    ...
    #   get_area_sqkm()
    #--------------------------------------------------------------------
    def ocean_outlet_count(self):
 
        df = self.catchment_df     
  
        recs = df[df['toid'] == 'nex-0']
        count = len(recs)
